...
const createFilosofo = async (req, res) => {
  try {
    const { title, year, corrente, ideia, descriptions } = req.body;
    await filosofoService.Create(title, year, corrente, ideia, descriptions);
...
const updateFilosofo = async (req, res) => {
  try {
    if (ObjectId.isValid(req.params.id)) {
      const id = req.params.id;
      const { title, year, corrente, ideia, descriptions } = req.body;
      const game = await gameService.Update(id, title, year, corrente, ideia, descriptions);
